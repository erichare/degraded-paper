...
ggplot(data = cor_compare, aes(x = value, color = match)) +
  stat_ecdf() +
  facet_wrap(~feature, nrow = 2) +
  theme_bw() + ylab("") + xlab("") +
  scale_color_brewer("Bullet Land Pairs", palette="Paired") +
...
p1 <- ggplot() +
  geom_line(data = filter(CCFs_tall, Type %in% c("True Positive")), aes(x = Feature, y = Value, colour = Type, group = id), alpha = 0.1, size = 0.2, linetype = "dashed") +
  geom_line(data = filter(CCFs_tall, Type %in% c("False Positive")), aes(x = Feature, y = Value, colour = Type, group = id), alpha = 1, size = 0.8, linetype = "dashed") +
...
ggplot(data = feature_exp2, aes(x = level, y = value, colour = fixed)) +
    geom_point() +
    geom_line() +
    theme_bw() +
    theme(legend.position = "bottom") +
